...
def run_test_case(question: str, expected_answer: str, history: List[Dict], category: str, course_name: str) -> Dict[str, any]:
    try:
        ...
        messages = [
            {"role": "system", "content": "You are an education assistant. Try to be as concise as possible. \n"}]
        messages.extend([convert_history_item(item) for item in history])
        ...
        chatbot_answer = completion.choices[0].message.content
